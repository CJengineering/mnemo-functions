...
export async function getAllDataChunks(_: Request, res: Response) {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to fetch data chunks" });
  }
}
...
export async function getDataChunkById(req: Request, res: Response) {
  try {
    ...
    const result = await pool.query("SELECT * FROM data_chunks WHERE id = $1", [
      id,
    ]);
    ...
    if (result.rows.length === 0) {
      ...
      return res
        .status(404)
        .json({ success: false, error: "Data chunk not found" });
    }
...
export async function updateDataChunk(req: Request, res: Response) {
  try {
    ...
    if (!name || !type || !data) {
      return res
        .status(400)
        .json({ success: false, error: "Missing required fields for update" });
    }

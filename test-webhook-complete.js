  ...
  try {
    ...
    if (createResult.success) {
      ...
      if (updateResult.success) {
        ...
        console.log(
          "   Changed fields should include: ['title', 'status', 'data']"
        );
      } else {
      ...
      if (
        createResult.error &&
        createResult.error.includes("Missing required fields")
      ) {
        console.log("\n🔍 Debugging Info:");
        console.log(
          "The createCollectionItemFromForm function expects IncomingCollectionItem format:"
        );
        console.log(
          "{ type: 'tag', data: { title, slug, name, 'name-arabic'?, status? } }"
        );
        console.log(
          "Your payload structure looks correct, check field values."
        );
      }
    }
  } catch (error) {
...
async function checkWebhookEnvironment() {
  ...
  if (willWebhooksRun) {
    ...
    console.log(
      "   Real HTTP calls will be made to Community Jameel endpoints"
    );
  } else {
...
(async () => {
  try {
    ...
    if (webhooksEnabled) {
      console.log(
        "  1. Check Community Jameel server logs for webhook requests"
      );
      ...
      console.log(
        "  3. Implement webhook endpoints on Community Jameel website"
      );
    }
